...
router.post('/resumenMovimientos', resumenMovimientos);
router.get('/:id', uno);
...
async function resumenMovimientos(req,res, next){
   try {
      const items = await controlador.resumenMovimientos(req.body) 
         respuestas.success(req, res, items, 200)       
   } catch (error) {
         next(error)
   }
